...
namespace EmoDic
{
    class DuLieuChuanHoa
    {
        ...
        static DuLieuChuanHoa PCentroid = new DuLieuChuanHoa(0.5948,1.0086,0.1724,0.181,0.1293);
        ...
        public float getPresentValue(List<CamXuc> khiacanh)
        {
            ...
            List<float> listTS = new List<float>();
            foreach(CamXuc cx in khiacanh)
            {
                listTS.Add(cx.trongso);
            }
            float kq = listTS.Min() + listTS.Max();
            if (kq == 0) return 0;
            return kq < 0 ? listTS.Min() : listTS.Max();
